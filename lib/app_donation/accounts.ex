defmodule AppDonation.Accounts do
  @moduledoc """
  ...
  alias AppDonation.Accounts.{User, UserToken, UserNotifier}
  ...
  See `AppDonation.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `AppDonation.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  defp create_organization_profile(user, attrs) do
    %AppDonation.Organizations.Organization{}
    |> AppDonation.Organizations.Organization.changeset(Map.put(attrs, "user_id", user.id))
    |> Repo.insert()
  end
  ...
  defp update_organization_profile(user, attrs) do
    profile = user.organization || %AppDonation.Organizations.Organization{user_id: user.id}
